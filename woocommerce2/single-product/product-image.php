...
?>
 ...
 <div class="product-image">
			...
			<?php
				...
				$image_link  	= woocommerce_watermark_image(wp_get_attachment_url( get_post_thumbnail_id() ));
				$image       	= woocommerce_watermark_image(get_the_post_thumbnail( $post->ID, apply_filters( 'single_product_large_thumbnail_size', 'shop_single' ), array(
					'title'	=> $image_title,
					'alt'	=> $image_title
					) ));
				...
				'<div class="slide easyzoom first is-selected"><a href="%s" itemprop="image" class="woocommerce-main-image zoom" title="%s"><img src="%s" alt="%s" /></a></div>',
				$image_link,
				$image_caption,
				...
				if ( $attachment_ids ) {
					...
					foreach ( $attachment_ids as $attachment_id ) {
						...
						<div class="slide easyzoom">
						  <a href="<?php echo woocommerce_watermark_image($src[0]); ?>" title="<?php echo $image_title; ?>">
	    					<img src="<?php echo $image_small[0]; ?>" data-flickity-lazyload="<?php echo woocommerce_watermark_image($image[0]) ?>" alt="<?php echo $image_title ?>" title="<?php echo $image_title ?>" />
	   				 	  </a>
	   				 	</div>
						<?php
					}
